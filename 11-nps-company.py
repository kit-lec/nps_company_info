# 출처: EP13. 기업의 월급여, 연봉 추정 서비스 제작 & 웹으로 배포 (무료 호스팅)
#  https://youtu.be/OpeECxk5c-Q?list=PLIMb_GuNnFweSpt4s8BhlN7EggZnWWqy6

# 사전필요: GitHub 계정
#          DropBox 계정  (파일 업로드)  (다른 대안 있으면 다르게 하자.)
...
# Streamlit 1.36 부터 @st.cache 는 deprecated  됨
# st.cache_data 나 st.cache_resource 사용하기
...
if data and company_name:
    ...
    if len(output) > 0:
        st.subheader(output.iloc[0]['사업장명'])   # 검색결과의 첫번째 사업장명을 제목으로
...
# 이를 github 에 배포까지 합니다
#   https://youtu.be/OpeECxk5c-Q?list=PLIMb_GuNnFweSpt4s8BhlN7EggZnWWqy6&t=467
